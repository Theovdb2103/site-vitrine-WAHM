...
export function DisciplinesCarousel({ items = [] }) {
  ...
  const arrow =
    'flex h-12 w-12 items-center justify-center border border-line/25 bg-wahm-navy/80 text-white backdrop-blur-sm transition-colors duration-200 hover:border-wahm-orange hover:bg-wahm-orange hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-wahm-goldLight md:h-14 md:w-14'
        ...
        <div className="-ml-4 flex">
          {items.map((d, i) => (
            <div key={d.img} className="min-w-0 shrink-0 grow-0 basis-[82%] pl-4 sm:basis-[55%] lg:basis-1/3">
              <article className="group relative aspect-[3/4] overflow-hidden border border-line/[0.12] bg-surface-2">
                <img
                  src={d.img}
                  alt={d.name}
                  loading="lazy"
                ...
                />
                <span aria-hidden="true" className="absolute inset-0" style={{ background: 'linear-gradient(0deg, rgb(var(--c-scrim) / 0.95) 0%, rgb(var(--c-scrim) / 0.30) 52%, rgb(var(--c-scrim) / 0.04) 100%)' }} />
                ...
                <div className="absolute inset-x-0 bottom-0 p-6 md:p-7">
                  ...
                  {d.tag && <p className="mt-2 font-mono text-[11px] uppercase tracking-[0.14em] text-white/70">{d.tag}</p>}
                </div>
              </article>
            </div>
          ))}
